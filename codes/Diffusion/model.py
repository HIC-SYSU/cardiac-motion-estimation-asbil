...
from diffusion.unet import Score_UNet, Unet
from VariationalAE import VariationalAutoEncoder
from Functions import crossCorrelation2D, gradientLoss, KullbackLeiblerDivergence, subplots
from Layers import ConvLayer, SpatialTransformer
import math
...
class LatentDiffusionmoprh(nn.Module):
    def __init__(self, inshape, indim, diffuse):
        super().__init__()

        # self.unet = UNetModel(image_size=128,
        #                       in_channels=1,
        #                       model_channels=224,
        #                       out_channels=1,
        ...
        self.deform = VariationalAutoEncoder(inshape, indim=8, hiddim=8)
        ...
        # loss
        self.lambda_L = 1
        self.loss_ncc = crossCorrelation2D(1, kernel=(9, 9))
        self.loss_reg = gradientLoss("l2")
        self.loss_func = nn.L1Loss(reduction='mean') if self.loss_type == 'l1' else nn.MSELoss(reduction='mean')
        self.kl = KullbackLeiblerDivergence(prior_lambda=10).kld_loss
    ...
    def forward(self, moving, fixed, condition=None, mode='train'):
        ...
        if mode in ['train', 'val']:
            ...
            x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)  # c:256
            ...
            D = self.deform('udecx',x_recon) # deformation field: net
            D = self.F(D) 
            ...
            pixel_wise_uncertainty, flows = self.uncertainty_map(moving, fixed, condition)
    ...
    def p_sample_loop(self, moving, fixed, condition=None):
        ...
        D = self.deform('udecx',fin_latent) # deformation field: net
        D = self.F(D)
    ...
    def uncertainty_map(self, moving, fixed, condition=None, M=10):
        ...
        with torch.no_grad():
            ...
            x_start = torch.cat([moving, fixed], dim=1)
            b, c, h, w = x_start.shape
            t = torch.Tensor([1000 for _ in range(b)]).long().cuda()
            noise = default(None, lambda: torch.randn_like(x_start))
            x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
            Condition = torch.cat([moving, fixed], dim=1) if condition is None else condition
            Condition = F.interpolate(Condition, size=(h,w), mode='bilinear', align_corners=True)
            input = torch.cat([Condition, x_noisy], dim=1)
            x_0 = self.unet(input, t) 
            D = self.deform('udecx',x_0) # deformation field: net
            flows = self.F(D) 
            ...
            for _ in range(M):
                ...
                x_1 = self.unet(input, t)   # denoising
                # x_1 = F.interpolate(x_1, size=(h,w), mode='bilinear', align_corners=True)
                D = self.F(self.deform('udecx',x_1))
                # warp_moving = self.transformer(m, D)
                E_t.append(D)
            E_t = torch.stack(E_t, dim=0)
            Et_var = torch.var(E_t, dim=0)
            pixel_wise_uncertainty = torch.mean(Et_var, dim=1, keepdim=True)
            # pixel_wise_uncertainty = Et_var
            # subplots(1-pixel_wise_uncertainty)

        return pixel_wise_uncertainty, flows


def model_save(model, optimizer, path=None):
    ...
    if path is not None:
        ...
        torch.save(main_checkpoint, path)
    else:
        pass
...
if __name__ == "__main__":
    ...
    train_imgpath = '/data/zhuangsx/cineMRI/train/videos' #/data/zhuangshuxin/Cardiac_tMRI
                                                       #/data/zhuangshuxin/4D_MRI/train/videos
    train_lbpath = '/data/zhuangsx/cineMRI/train/annotations'
    currrent_savepath = '/home/zhuangsx/code/3D_tracking/checkpoints/diffusemorph.pth'
    dimention = '3D'
    max_skip = 8
    batchsize = 2
    # 数据集
    train_dataset = VOSDataset(train_imgpath, train_lbpath, max_skip, mode='val', dim=dimention)
    ...
    # 模型
    model = LatentDiffusionmoprh([80,80]).cuda()    #1e-3, no more
    Optimizer = torch.optim.Adam(model.parameters(), lr=5e-4)
    total_epoch = range(1000) 
    prev_ls = 999 
    best_e = 0
    # 训练
    model.train()
    ...
    for e in total_epoch:
        ...
        for data in tqdm(vos_dataset):
            ...
            Optimizer.zero_grad()
            score, loss = model.forward(images)
            score, out_M, flow = score
